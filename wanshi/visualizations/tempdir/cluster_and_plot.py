...
class DataClustering:
    ...
    def get_feature_label(self, X_2d, y):
        ...
        self.df2['comp-1']=(X_2d[:, 0])
        self.df2['comp-2']=(X_2d[:, 1])
    ...
    def plot_scatter(self, plot_name, title, size=1, marker='o'):
        plt.figure(figsize=(16, 10))
        n_classes = len(self.df2['labels'].unique())
        sns.scatterplot(x='comp-1', y='comp-2', hue=self.df2.labels.tolist(),
                        palette=sns.color_palette('hls', n_classes),
                        data=self.df2, s=size, marker = marker).set(title=title)
        # plt.show()
        self.plot_dir.mkdir(exist_ok=True)
        path_scatter_dots = str(self.plot_dir / plot_name) + title
